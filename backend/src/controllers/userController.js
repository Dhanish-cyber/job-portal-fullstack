...
exports.getAdminStats = async (req, res) => {
  try {
    ...
    const recentUsers = await query.all(`SELECT id, name, email, role FROM users ORDER BY created_at DESC LIMIT 20`);
    const recentJobs = await query.all(`SELECT id, title, company_name, location, status FROM jobs ORDER BY created_at DESC LIMIT 20`);
